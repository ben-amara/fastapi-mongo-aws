...
async def create_shorten(request: Request, shorten: ShortenModel = Body(...), user_collect: dict = Depends(_get_authorization_token)):    
    shorten = jsonable_encoder(shorten)    
    new_shorten = await add_shorten(shorten, user_collect)
